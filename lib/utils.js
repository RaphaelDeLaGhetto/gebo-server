 ...
 * 2014-8-20
 *
 * I'm moving away from saving files to the agent directory in favour of
 * storing all files in mongo. This is preserved for reference.
 *
/**
...
//    /**
//     * 2014-4-17
//     * Why does this need to be placed here? Previously,
//     * I could load the file at the top of the page (see
//     * commented above), and everything would work fine 
//     */
//    nconf.file({ file: 'gebo.json' });
//    var dir = nconf.get('docs') + '/' + verified.resource;
//
//    if (index > 0) {
 ...
 * @return promise
 */
//function _saveFile(file, verified) {
//   var deferred = q.defer();
//
...
//    var dir = nconf.get('docs') + '/' + verified.resource;
//
//    _getSafeFileName(file.name, dir).
//        then(function(filename) {
//
//            // Make file document for DB
//            var db = require('../schemata/agent')();
//            var fileObj = new db.fileModel({
//                    name: filename,
//                    resource: verified.resource,
//                    type: file.type,
//                    size: file.size,
//                }); 
//                        
//            fileObj.save(function(err, savedFile) {
//                if (err) {
//                 deferred.reject(err);
//                }
//                else {
//                  mv(file.path, dir + '/' + filename, { mkdirp: true },
//                      function(err) {
//                          if (err) {
//                            deferred.reject(err);
//                          }
...
function _saveFileToDb(file, collection) {
    ...
    // Open a new file
    var gridStore = new GridStore(collection.db, new ObjectID(), file.name, 'w', { metadata: { collection: collection.collectionName } });
